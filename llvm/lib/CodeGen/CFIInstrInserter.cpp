   ...
   struct CSRSavedLocation {
     ...
     void dump(raw_ostream &OS) const {
       ...
       case Kind::REGISTER:
         ...
         break;
       case Kind::CFA_OFFSET:
         OS << "At CFA offset: (fixed: " << Offset.getFixed() << ", scalable: " << Offset.getScalable() << ")";
         break;
       case Kind::REG_OFFSET:
         OS << "At offset " << Offset.getFixed() << " from Dwarf register " << Reg;
         break;
       }
     }
   };
  ...
  for (MachineInstr &MI : *MBBInfo.MBB) {
    if (MI.isCFIInstruction()) {
      ...
      case MCCFIInstruction::OpLLVMDefCfaRegScalableOffset: {
        ...
        OutgoingCFAOffset = StackOffset::get(CFI.getFixedOffset(), CFI.getScalableOffset());
        break;
      }
      ...
      case MCCFIInstruction::OpLLVMRegAtScalableOffsetFromCfa: {
        CSRSavedLocation &CSRLocation = OutgoingCSRLocations[CFI.getRegister()];
        CSRLocation.K = CSRSavedLocation::Kind::CFA_OFFSET;
        CSRLocation.Offset = StackOffset::get(CFI.getFixedOffset(), CFI.getScalableOffset());
        break;
      }
      ...
      case MCCFIInstruction::OpRelOffset: {
        CSRSavedLocation &CSRLocation = OutgoingCSRLocations[CFI.getRegister()];
        CSRLocation.K = CSRSavedLocation::Kind::CFA_OFFSET;
        CSRLocation.Offset = StackOffset::getFixed(CFI.getOffset()) - OutgoingCFAOffset;
        break;
      }
...
bool CFIInstrInserter::insertCFIInstrs(MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    if ((PrevMBBInfo->OutgoingCFAOffset != MBBInfo.IncomingCFAOffset &&
      ...
      // section, add a def_cfa (or llvm_def_cfa_reg_scalable_offset) instruction with the correct offset and
      ...
      if (CorrectOffset.getScalable() == 0) {
        ...
        CFIIndex = MF.addFrameInst(MCCFIInstruction::createLLVMDefCfaRegScalableOffset(
          nullptr, MBBInfo.IncomingCFARegister, CorrectOffset.getScalable(), CorrectOffset.getFixed()));
      }
    ...
    for (unsigned i = 0; i < PrevMBBInfo->OutgoingCSRLocations.size(); ++i) {
      ...
      if (HasToBeCSRLoc.K == CSRSavedLocation::Kind::CFA_OFFSET &&
          ...
          if (CorrectOffset.getScalable() == 0) {
            ...
            MCCFIInstruction::createOffset(nullptr, i, CorrectOffset.getFixed()));
            } else {
              CFIIndex = MF.addFrameInst(
              MCCFIInstruction::createLLVMRegAtScalableOffsetFromCfa(nullptr, i, CorrectOffset.getScalable(), CorrectOffset.getFixed()));
            }
